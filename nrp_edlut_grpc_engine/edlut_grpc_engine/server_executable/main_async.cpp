...
int main(int argc, char * argv[])
{
    ...
    json_utils::setDefault<std::string>(config.at("EngineConfig"), "EngineType", EdlutGrpcConfigConst::EngineType);
    ...
    EdlutEngineGrpcClient client(config.at("EngineConfig"), ProcessLauncherInterface::unique_ptr{});
    ...
    // Instantiate Engine
    auto wrapper = new EdlutEngine(client.engineName(), protobufPluginsPath, protobufPlugins);
    EventLoopEngine engine(timestep, timestepWarn,
                           config.at("DataQueueSize").get<size_t>(),
                                   config.at("ProcessLastMsg").get<bool>(),
                                           client.engineConfig(), wrapper, true, logRTInfo, useGlobalClock,
                                           publishDatapacks);
