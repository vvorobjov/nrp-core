...
xvfb-run --auto-servernum gzserver --protobuf_plugins_path=${NRP_INSTALL_DIR}/lib --protobuf_plugins="[\"Gazebo\"]"  ${GAZEBO_ADDITIONAL_PARAMETERS} --engine=gazebo --serverurl=0.0.0.0:"${GAZEBO_PORT}" -s "${GAZEBO_NRP_PLUGINS}" --seed "${GAZEBO_RNG_SEED}" "${GAZEBO_WORLD_FILE}"
