...
class NrpCore:
    ...
    def _init_protobuf_classes(self) -> None:
        ...
        for module_name in nrp_proto_modules:
            ...
            modules.append(mod)
            file_names.append(mod.DESCRIPTOR.name)

        pool = descriptor_pool.Default()

        # (Usually unnecessary, but safe) ensure files are in the pool
        # in case the default import didn't register them:
        for mod in modules:
            try:
                pool.FindFileByName(mod.DESCRIPTOR.name)
            except KeyError:
                pool.AddSerializedFile(mod.DESCRIPTOR.serialized_pb)

        # This returns { "package.Message": PythonClass, ... }
        self.proto_message_classes = GetMessageClassesForFiles(file_names, pool)
