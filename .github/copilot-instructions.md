...
The command that gates acceptance is:
...
bash .ci/30-run-tests.sh         # ctest — MUST finish with exit 0
```

If you cannot run those three commands — because the environment is missing,
the container isn't built, or something else — say so explicitly in your PR
description and do **not** mark the change as ready. Suggest a reviewer run
them on your behalf.
...
- [ ] `bash .ci/11-prepare-build.sh` succeeds with no new warnings.
- [ ] `bash .ci/20-build.sh` completes.
- [ ] `bash .ci/30-run-tests.sh` exits 0.
