...
#ifndef ${engine_name_uppercase}_GRPC_DEVICE_CONTROLLER_SERVER_H
#define ${engine_name_uppercase}_GRPC_DEVICE_CONTROLLER_SERVER_H
...
class ${engine_name}GrpcDeviceController
    : public DeviceController<google::protobuf::Message>
{
	public:
		${engine_name}GrpcDeviceController(const std::string & deviceName,
		 ...
		 */
		void handleDeviceData(const google::protobuf::Message &data) override;

		/*!
		 * \brief Returns the newest simulation data
		 *
		 * The data will be passed to the engine client through gRPC.
		 * There it will be wrapped in a device object and passed to the transceiver functions.
		 *
		 ...
		 */
		google::protobuf::Message * getDeviceInformation() override;

	private:

		/*!
		 * \brief Name of the device that is handled by this controller object
		 */
...
#endif // ${engine_name_uppercase}_GRPC_DEVICE_CONTROLLER_SERVER_H
