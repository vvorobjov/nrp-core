...
${engine_name}JSONDataPackController::${engine_name}JSONDataPackController(const std::string & datapackName,
...
void ${engine_name}JSONDataPackController::handleDataPackData(const nlohmann::json &data)
{
    std::cout << "TF output data: " << data["data"]["test_string"] << std::endl;
    setCachedData(data);
}

nlohmann::json * ${engine_name}JSONDataPackController::getDataPackInformation()
{
    static int data = 0;
